...
const FLAG = {
  Cyprus: '🇨🇾', China: '🇨🇳', Japan: '🇯🇵', 'United Kingdom': '🇬🇧', UK: '🇬🇧',
  Australia: '🇦🇺', Germany: '🇩🇪', France: '🇫🇷', Spain: '🇪🇸', Italy: '🇮🇹',
  Canada: '🇨🇦', USA: '🇺🇸', 'United States': '🇺🇸', Brazil: '🇧🇷', Portugal: '🇵🇹',
}
...
function NavItem({ href, icon, label, sub, active }) {
  return <a href={href} className={active ? styles.navActive : ''}><span className={styles.navIcon}><Icon type={icon} size={17}/></span><span><strong>{label}</strong><small>{sub}</small></span></a>
}
...
function Sparkline({ data, dataKey, tone }) {
  if (!data.length) return <div className={styles.sparkEmpty} />
  return <div className={styles.sparkline}><ResponsiveContainer width="100%" height="100%"><LineChart data={data}><Line type="monotone" dataKey={dataKey} stroke={tone} strokeWidth={2} dot={false} isAnimationActive={false}/></LineChart></ResponsiveContainer></div>
}

function Kpi({ label, value, previous, tone, icon, sparkData, sparkKey }) {
  const change = delta(value, previous)
  return <div className={`${styles.kpi} ${styles[`kpi_${tone}`]}`}>
    <div className={styles.kpiMain}><span className={styles.kpiIcon}>{icon}</span><div><span>{label}</span><strong>{fmt(value)}</strong></div><Sparkline data={sparkData} dataKey={sparkKey} tone={tone === 'red' ? '#ff6173' : tone === 'green' || tone === 'teal' ? '#31e6b5' : '#2bc8ff'}/></div>
    <div className={`${styles.kpiDelta} ${change != null && change < 0 ? styles.deltaDown : ''}`}>{deltaLabel(change)}</div>
  </div>
}
...
export default function RegFunnelDashboard({ onBack, user, onLogout }) {
  ...
  const filteredAbStats = (data.abStats || []).filter((row) =>
  ...
  const lift = aMetric && bMetric != null
    ? (abMetric === 'time' ? ((aMetric - bMetric) / aMetric) * 100 : ((bMetric - aMetric) / aMetric) * 100)
    : null
  ...
  const conversionByStep = useMemo(() => {
    const map = new Map()
    for (const row of convertedByStep) map.set(row.step_id, (map.get(row.step_id) || 0) + Number(row.n || 0))
  ...
  return <div className={styles.shell}>
    <aside className={styles.sidebar}>
      ...
      <div className={styles.sidebarQuote}><strong>Smarter Journeys.<br/>Higher Impact.</strong><span>Automate. Learn. Convert.</span></div>
    </aside>

    <main className={styles.main} id="overview">
      <div className={styles.topbar}>
        <label className={styles.search}><Icon type="search" size={16}/><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search users, enrollments or steps…"/></label>
        <div className={styles.topFilters}>
          <label className={styles.dateSelect}><Icon type="calendar" size={15}/><select value={days} onChange={(e) => setDays(Number(e.target.value))}>{RANGES.map((v) => <option key={v} value={v}>Last {v} days</option>)}</select></label>
          ...
          <button className={styles.alertBtn} title={alerts ? `${alerts} operational signal(s) need review` : 'No current send or sequence alerts'}><Icon type="bell" size={17}/>{alerts > 0 && <i />}</button>
          <div className={styles.userBox}><div className={styles.avatar}>{String(user?.name || user?.username || 'M').charAt(0).toUpperCase()}</div><div><strong>{user?.name || user?.username || 'Marketing Ops'}</strong><small>Marketing Ops</small></div></div>
          {onBack && <button className={styles.appsBtn} onClick={onBack}>Apps</button>}
          {onLogout && <button className={styles.logoutBtn} onClick={onLogout} title="Sign out">↗</button>}
        </div>
      </div>

      <header className={styles.hero}><div><h1>RegFunnelOps</h1><p>Monitor. Optimize. Move people forward.</p></div><span>LESS FRICTION. MORE PEOPLE FORWARD.</span></header>
      ...
      <section className={styles.topGrid}>
        <Panel title="Funnel Journey" sub="High-level progression across the registration sequence" className={styles.journeyPanel} action={<button className={styles.textAction} onClick={() => setShowSequence((value) => !value)}>{showSequence ? 'Hide email steps' : 'View full journey'} →</button>}>
          <div className={styles.stageFlow} id="journey">{stages.map((stage, index) => <div className={`${styles.stage} ${index === 0 ? styles.stageActive : ''}`} key={stage.key}><span>{index + 1}</span><strong>{stage.label}</strong><b>{fmt(stage.value)}</b><small>{stage.rate.toFixed(0)}%</small></div>)}</div>
          ...
          <div className={styles.miniChart}>{trend.length > 1 ? <ResponsiveContainer width="100%" height={96}><BarChart data={trend.slice(-24)}><Bar dataKey="enrolled" fill="#28d6b4" radius={[3,3,0,0]}/></BarChart></ResponsiveContainer> : <div className={styles.chartEmpty}>More enrollment dates are needed to build a trend.</div>}</div>
        </Panel>

        <Panel title="Send Health" sub="Delivery and send outcomes" className={styles.sendPanel} action={<span>{pct(delivered, sent)} delivered</span>}>
          <div className={styles.sendHealth} id="send-health"><div className={styles.donut}><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={sendHealth} dataKey="value" innerRadius={55} outerRadius={76} paddingAngle={2}>{sendHealth.map((item) => <Cell key={item.name} fill={item.tone}/>)}</Pie></PieChart></ResponsiveContainer><div className={styles.donutCenter}><strong>{fmt(totalAttempts)}</strong><span>Total attempts</span></div></div><div className={styles.legend}>{sendHealth.map((item) => <div key={item.name}><i style={{background:item.tone}}/><span>{item.name}<small>{pct(item.value,totalAttempts)}</small></span><strong>{fmt(item.value)}</strong></div>)}</div></div>
          <div className={`${styles.healthNote} ${sendErrors > 0 ? styles.healthWarn : ''}`}>{sendErrors > 0 ? `${sendErrors} send error(s) need review.` : sent > 0 && delivered === 0 ? 'Sends are healthy. Delivery events are not recorded yet.' : 'Send health is stable.'}</div>
        </Panel>

        <Panel title="A/B Test Performance" sub="Real sequence-level outcomes" className={styles.abPanel}>
          <div className={styles.metricTabs} id="ab"><button className={abMetric === 'conversion' ? styles.tabActive : ''} onClick={() => setAbMetric('conversion')}>Conversion Rate</button><button className={abMetric === 'completion' ? styles.tabActive : ''} onClick={() => setAbMetric('completion')}>Completion Rate</button><button className={abMetric === 'time' ? styles.tabActive : ''} onClick={() => setAbMetric('time')}>Time to Convert</button></div>
          <div className={styles.abList}>{abRows.map((row) => {const value=metricValue(row);const width=abMetric==='time'?0:Math.min(100,Number(value||0));return <div className={styles.abRow} key={row.variant}><div className={`${styles.variantBadge} ${row.variant === 'B' ? styles.variantB : ''}`}>{row.variant}</div><div className={styles.abBody}><div><strong>Variant {row.variant}</strong><span>{row.variant === 'A' ? 'Current (Control)' : 'Challenger'}</span></div>{abMetric !== 'time' && <div className={styles.progress}><i style={{width:`${width}%`}}/></div>}<small>{abMetric === 'time' ? `${fmt(row.converted)} converted` : `${fmt(abMetric === 'completion' ? row.completed : row.converted)} / ${fmt(row.enrolled)}`}</small></div><strong>{abMetric === 'time' ? hoursLabel(value) : `${Number(value||0).toFixed(1)}%`}</strong>{row.variant === 'B' && lift != null && <em className={lift >= 0 ? styles.liftGood : styles.liftBad}>{lift >= 0 ? '+' : ''}{lift.toFixed(1)}%</em>}</div>})}</div>
          <div className={styles.abFoot}>{abRows.reduce((t,r)=>t+r.enrolled,0) < 30 ? 'Sample is still too small for a reliable winner.' : lift == null ? 'Both variants need measurable results before lift can be calculated.' : `Variant B ${lift >= 0 ? 'is ahead' : 'is behind'} by ${Math.abs(lift).toFixed(1)}% on this metric.`}</div>
        </Panel>
      </section>

      <section className={styles.midGrid}>
        <Panel title="Performance Over Time" sub="Filtered enrollment cohort trend" className={styles.performancePanel} action={<div className={styles.performanceAction}><select value={trendMetric} onChange={(e) => setTrendMetric(e.target.value)}><option value="enrolled">Enrollments</option><option value="converted">Conversions</option><option value="completed">Completions</option><option value="excluded">Exclusions</option></select><span className={delta(currentMetricValue, previousMetricValue) != null && delta(currentMetricValue, previousMetricValue) < 0 ? styles.deltaDown : styles.deltaUp}>{deltaLabel(delta(currentMetricValue, previousMetricValue))}</span></div>}>
          {trend.length > 1 ? <ResponsiveContainer width="100%" height={235}><LineChart data={trend}><defs><linearGradient id="rfLineFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#2bc8ff" stopOpacity=".35"/><stop offset="100%" stopColor="#2bc8ff" stopOpacity="0"/></linearGradient></defs><CartesianGrid stroke="rgba(255,255,255,.06)" vertical={false}/><XAxis dataKey="day" tick={{fill:'#718ba1',fontSize:10}} minTickGap={26}/><YAxis tick={{fill:'#718ba1',fontSize:10}} width={34}/><Tooltip contentStyle={{background:'#0b2035',border:'1px solid #1c3b56',borderRadius:10}}/><Line type="monotone" dataKey={trendMetric} stroke="#29c9ff" strokeWidth={2.4} dot={{r:2,fill:'#29c9ff'}} activeDot={{r:4}}/></LineChart></ResponsiveContainer> : <div className={styles.chartEmptyLarge}><strong>Trend not available yet</strong><span>Your test cohort has only one enrollment date. This chart will populate automatically as new users enroll.</span></div>}
        </Panel>

        <Panel title="Top Insights" sub="Live operational signals" className={styles.insightsPanel}><div className={styles.insights}><div><i className={styles.good}>↗</i><span><strong>Conversion rate</strong><small>{pct(stateCounts.converted,enrolled)} of the selected cohort has converted.</small></span><em>Live</em></div><div><i className={sendErrors ? styles.bad : styles.good}>!</i><span><strong>Send errors</strong><small>{sendErrors ? `${sendErrors} error(s) recorded in SendLog.` : 'No send errors in this cohort.'}</small></span><em>Live</em></div><div><i className={sent > 0 && delivered === 0 ? styles.bad : styles.info}>i</i><span><strong>Delivery tracking</strong><small>{sent > 0 && delivered === 0 ? 'Emails were sent, but delivery events have not been captured yet.' : `${fmt(delivered)} delivered event(s) recorded.`}</small></span><em>Live</em></div><div><i className={legacySteps.length ? styles.bad : styles.info}>✓</i><span><strong>Sequence integrity</strong><small>{legacySteps.length ? `${legacySteps.length} legacy step row(s) detected.` : 'All engagement rows match the live sequences.'}</small></span><em>Live</em></div></div></Panel>

        <Panel title="Regional Performance" sub="Conversion rate by country" className={styles.regionPanel} action={<span>Conversions</span>}><div className={styles.regionList}>{countryRows.length ? countryRows.map((row) => <div key={row.country}><span><b>{FLAG[row.country] || '🌐'}</b>{row.country}</span><div className={styles.regionBar}><i style={{width:`${Math.min(100,row.rate)}%`}}/></div><strong>{row.rate.toFixed(1)}%</strong></div>) : <div className={styles.noCountry}>No country data in this cohort.</div>}</div></Panel>
      </section>
      ...
      <Panel title="Recent Enrollments" sub="Search and inspect the latest users in the selected cohort" className={styles.enrollmentPanel} action={<div className={styles.tableActions}><button onClick={exportCsv}><Icon type="export" size={14}/> Export</button><a href="/admin/collections/funnel-enrollment">View all enrollments →</a></div>}>
        <div className={styles.tableWrap} id="enrollments"><table><thead><tr>{[['external_id','External ID'],['country','Country'],['funnel_region','Region'],['os_app','App'],['variant','Variant'],['state','State'],['current_step','Current Step'],['next_send_at','Next Send'],['enrolled_at','Enrolled At']].map(([key,label]) => <th key={key}><button onClick={() => toggleSort(key)}>{label}<span>{sortKey === key ? (sortDir === 'asc' ? '↑' : '↓') : '↕'}</span></button></th>)}<th /></tr></thead><tbody>{!visibleRecent.length && <tr><td colSpan={10} className={styles.empty}>{search ? 'No recent enrollments match your search.' : 'No enrollments in this cohort.'}</td></tr>}{visibleRecent.map((row) => <tr key={row.external_id}><td><strong>{row.external_id}</strong></td><td><span className={styles.countryCell}>{FLAG[row.country] || '🌐'} {row.country || '—'}</span></td><td>{row.funnel_region === 'CNJP' ? 'CN / JP' : row.funnel_region}</td><td>{APP_LABELS[row.os_app] || row.os_app}</td><td><b className={styles.variantText}>{row.variant}</b></td><td><StatusPill state={row.state}/></td><td title={row.current_step}>{shortStep(row.current_step)}</td><td className={row.next_send_at && new Date(row.next_send_at) < new Date() ? styles.overdue : ''}>{relativeDue(row.next_send_at)}</td><td>{dateTime(row.enrolled_at)}</td><td><a className={styles.rowAction} href={`/admin/collections/funnel-enrollment/${row.id}`} title="Open enrollment">•••</a></td></tr>)}</tbody></table></div>
